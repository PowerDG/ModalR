...
layuiXtree.prototype.getChildByClassName = function (obj, cn) {
    ...
    for (var i = 0; i < _xlist.length; i++) {
        var _xchild = _xlist[i];
        var _xclassNames = _xchild.getAttribute('class');
        if (_xclassNames === null) continue;
        _xclassNames = _xclassNames.split(' ');
        for (var j = 0; j < _xclassNames.length; j++) {
            if (_xclassNames[j] == cn) {
                _xtemp.push(_xchild);
                break;
...
layuiXtree.prototype.Loading = function (options) {
    ...
    _this.xloading.style.marginLeft = _this._container.offsetWidth / 2 + 'px';
...
//        _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox layui-xtree-ckall" title="全选" lay-skin="primary" lay-filter="xtreeckall' + _this.id + '">';
//        _this._domStr += '</div>';
//    }
//}
...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (i in d) {
            ...
            //+ xtree_isdisabled
            _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked + ' value="' + d[i].id + '" title="' + d[i].name + '" lay-skin="primary" lay-filter="xtreeck' + _this.id + '">';
...
layuiXtree.prototype.Rendering = function () {
    ...
    for (var i = 0; i < xtree_items.length; i++) {
        if (xtree_items[i].parentNode == _this._container)
            ...
            if (xtree_items[i].childNodes.length > 3)
                xtree_items[i].style.margin = '5px 0 0 15px';
            else
                xtree_items[i].style.margin = '5px 0 0 30px';
    ...
    _this._form.on('checkbox(xtreeck' + _this.id + ')', function (da) {
    ...
    ////全选按钮
    //if (_this._ckall) {
    //    _this._form.on('checkbox(xtreeckall' + _this.id + ')', function (data) {
...
layuiXtree.prototype.ParendCheck = function (ckelem) {
    ...
    if (xtree_p.getAttribute('class') == 'layui-xtree-item') {
        ...
        var xtree_count = 0;
        var parentCheck = _this.getChildByClassName(xtree_p, 'layui-form-checkbox')[0];
        var parentIcon = _this.getChildByClassName(parentCheck, 'layui-icon')[0];
        for (var i = 0; i < xtree_all.length; i++) {
            var childCheck = _this.getChildByClassName(xtree_p, 'layui-form-checkbox')[0];
            var childIcon = _this.getChildByClassName(childCheck, 'layui-icon')[0];
            if (_this.getChildByClassName(xtree_all[i], 'layui-xtree-checkbox')[0].checked && childIcon.style.color !== "#5FB878") {
                xtree_count++;
            }
        }
        parentIcon.style.color = "#fff";
        if (xtree_count <= 0) {
            ...
            if (xtree_count !== xtree_all.length) parentIcon.style.color = "#5FB878";
        }
